...
- The word being sung **lifts rather than scales** (`KaraokeWord.swell`, plus `glowStrength` for its halo), by a fraction of a pixel at the peak: both envelopes are time-based and reach zero at the word's own end, because SwiftUI re-rasterizes scaled text every frame (and snaps hardest as it returns to rest — 40 ms before the line ends, in the middle of the line's scale-down, which read as the line jumping in place), and because a glow that vanishes with its word is a jump too
