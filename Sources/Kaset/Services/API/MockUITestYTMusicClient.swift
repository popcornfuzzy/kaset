...
final class MockUITestYTMusicClient: YTMusicClientProtocol {
    ...
    func getPodcastTranscript(videoId _: String, preferredLanguageCode _: String?) async throws -> PodcastTranscript {
        try? await Task.sleep(for: .milliseconds(100))
        return PodcastTranscript(
            lines: [
                PodcastTranscriptLine(timeInMs: 0, durationMs: 12000, text: "This is a mock transcript paragraph for UI testing."),
                PodcastTranscriptLine(timeInMs: 12000, durationMs: 12000, text: "A second paragraph shows how the transcript scrolls."),
            ],
            languageCode: "en",
